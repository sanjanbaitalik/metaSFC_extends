...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=0.025,
        help=(
            "Shift panels (b) and (d) left by this fraction of the figure "
            "width to increase clearance from the colorbar."
        ),
    )
    ...
    # Finalize the panel geometry before Nilearn creates its internal axes.
    # Moving only the right column gives panels (b) and (d) more clearance
    # from the colorbar without changing the left-column layout.
    fig.subplots_adjust(
        left=0.01,
        right=0.92,
        top=0.96,
    ...
    for ax in axes[:, 1]:
        ...
        ax.set_position(
            [
                pos.x0 - args.right_column_shift,
                pos.y0,
                pos.width,
                pos.height,
            ]
    ...
    sm.set_array([])
    # Use a dedicated colorbar axis so adding the colorbar cannot resize or
    # pull the brain panels back toward it.
