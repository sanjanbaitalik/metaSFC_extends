...
The ROI-level meta-analysis prior enters the *attention logits* directly
(hard inductive bias on message routing), with gamma a learnable temperature
(see src/metascfc/models/iclr_backbones/meta_gat.py).  The split graph is
the row-wise top-10% positive threshold of the group-average structural
connectome over the inner training partition (leakage-free, self-loops).

Methods
-------
  M2_TRUE      true working-memory prior (Method 2)
  M2_SHUFFLED  anatomically shuffled control
  M2_RANDOM    random control

The (hidden, dropout, lr) grid is selected only on the inner validation
split; the winner is refit on train+val and evaluated on the outer test
split.  True/shuffled/random variants share identical grids, seeds, and
folds.  Node-level saliency (mean |dy/dx_i| over fit subjects, min-max
normalized) is exported per split for the biomarker alignment / rank
stability pipeline.

Outputs (identical schema to scripts/40_run_network_constrained_ridge.py):
...
from metascfc.models.iclr_backbones import fit_predict_meta_gat
...
def main() -> None:
    ...
    ap.add_argument("--config", default="configs/aaai/meta_gat.yaml")
    ...
    out_dir = Path(cfg.get("output_dir", "outputs/aaai/meta_gat"))
    ...
    for seed, fold, train_idx, val_idx, test_idx in iter_nested_splits(y, seeds, n_folds, val_fraction, groups):
        ...
        for method_id in selected_method_ids:
            if (method_id, seed, fold) in completed:
                ...
                continue
            started = time.time()
            pred, best_cfg, best_val_rmse, best_epoch, saliency, n_params = fit_predict_meta_gat(
            ...
            row = {
                ...
                "method_family": "prior_gated_gat", "seed": seed, "fold": fold,
